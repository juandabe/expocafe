<?php
    ...
    try
    {
        $connection_samanta = new PDO("odbc:Driver={SQL Native Client};Server=192.168.35.100;Database=SAMANTA; Uid=sa;Pwd=c0pc2013;"); 
    }
    ...
    {
            $valores="('".$i."', '".$rows[$i]['LOTE']."','".$rows[$i]['EXPORTADOR']."','".$rows[$i]['PROFORMA']."','".$rows[$i]['TOTALPESO']."','".$rows[$i]['RPSSACOS']."','".
                            $rows[$i]['RPSCONTENEDOR']."','".$rows[$i]['FECHA']."','".$rows[$i]['RPSSEQ']."','".$rows[$i]['RPSHORA']."','".$rows[$i]['NOTONAVE']."',
                            '".$rows[$i]['TOTALSACOS']."'),";

            if($i==$totalrows) {
                $valores="('".$i."', '".$rows[$i]['LOTE']."','".$rows[$i]['EXPORTADOR']."','".$rows[$i]['PROFORMA']."','".$row[$i]['TOTALPESO']."','".$rows[$i]['RPSSACOS']."','".
                            $rows[$i]['RPSCONTENEDOR']."','".$rows[$i]['FECHA']."','".$rows[$i]['RPSSEQ']."','".$rows[$i]['RPSHORA']."','".$rows[$i]['NOTONAVE']."',
                            '".$rows[$i]['TOTALSACOS']."')";
            }
